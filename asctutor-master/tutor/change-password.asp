        ...
        <%       	
            ...
            UserCurrPassword = getPass(Request.Cookies("user")("username"))
                ...
                If sha256(Request.Form("currPass") & "xxMc4ppl3saUceBurgerKingxx") <> UserCurrPassword Then
					Response.write("<div class='alert alert-error'>You entered your current password incorrectly or have a corrupted cookie, please try again.</div>")
				Else
...
function validateForm()
{
    ...
    while (length--)
    {
        if (username.indexOf(substrings[length]) !== -1 || currPass.indexOf(substrings[length]) !== -1 || newPass.indexOf(substrings[length]) !== -1 || confirmNewPass.indexOf(substrings[length]) !== -1)
        {
            alert("You entered an illegal character, try again.");
            return false;
        }
    }
}
